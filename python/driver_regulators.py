...
def driver_regulators(GRN_nx: nx.DiGraph,
    ...
    # The final driver regulators identified by DGRN
    DGRN_drivers = driver_set.intersection(critical_genes)

    drivers_df = gene_influence_score.loc[
        list(MFVS_driver_set.union(MDS_driver_set).union(critical_genes)),
        ['influence_score']
    ].copy()
    drivers_df['is_driver_regulator'] = drivers_df.index.isin(list(DGRN_drivers))
