...
class DGRNResults:
    ...
    def __repr__(self) -> str:
        descr = f"DGRNResults object with n_cells * n_genes = {self.n_cells} * {self.n_genes}, n_edges = {self.n_edges}"
        descr += f"\n    name: {self.name}" \
                 f"\n    expression_data: yes" \
    ...
    def RGM_activity(self, num_workers: int = 8, return_value: bool = False):
        ...
        if self.driver_regulator is None:
            raise ValueError(
                f'Did not find the result of driver regulators. Run `LineageNet.NetModel.driver_regulators` first.'
            )
    ...
    def plot_gene_embedding_with_clustering(self,
        ...
        """
        Cluster genes with DGRN derived embeddings by using the Leiden algorithm and visualize in 2-D UMAP space.
        """
        ...
        with rc_context({'figure.figsize': (4, 4)}):
            sc.pl.umap(self._adata_gene, color=['leiden'], legend_loc='on data',
                       legend_fontsize=8, legend_fontoutline=2, title='Leiden clustering using DGRN derived gene embeddings')
    ...
    def plot_RGM_activity_heatmap(self, cell_label: Optional[pd.DataFrame] = None, type: str = 'out'):
        ...
        if self.RGMs_AUCell_dict is None:
            raise ValueError(
                f'Did not find the result of RGMs. Run `LineageNet.NetModel.RGM_activity` first.'
            )
